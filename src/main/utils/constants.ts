...
export const DATA_DIR = process.env.PIXELCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "PixelcordData", suffix)
        : join(app.getPath("userData"), "..", "Pixelcord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Pixelcord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
