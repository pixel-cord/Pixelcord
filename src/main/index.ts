...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("pixelcord", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("pixelcord://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Pixelcord] Installed React Developer Tools"))
                    .catch(err => console.error("[Pixelcord] Failed to install React Developer Tools", err));
        } catch { }
